...
public class ConfiguredFiddleHttpClient {
	...
	public ConfiguredFiddleHttpClient(final HttpClient client, final String url, Optional<Map<String, String>> headers) {
		this.client = client;
		this.url = url;
		this.headers = headers;
	}

	public ConfiguredFiddleHttpClient withHeader(final String type, final String value) {
		final Map<String, String> headers = new HashMap<String, String>(this.headers.or(Maps.<String, String>newHashMap()));
		headers.put(type, value);
		
		return new ConfiguredFiddleHttpClient(client, url, Optional.of(headers));
	}
	
	public ConfiguredFiddleHttpClient withHeaders(final Map<String, String> headers) {
		return new ConfiguredFiddleHttpClient(client, url, Optional.fromNullable(headers));
	}
